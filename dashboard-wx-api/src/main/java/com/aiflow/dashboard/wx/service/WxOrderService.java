...
import com.aiflow.dashboard.core.express.ExpressService;
import com.aiflow.dashboard.core.express.dao.ExpressInfo;
import com.aiflow.dashboard.core.notify.NotifyService;
import com.aiflow.dashboard.core.notify.NotifyType;
...
import com.aiflow.dashboard.core.util.DateTimeUtil;
...
import com.aiflow.dashboard.db.util.OrderHandleOption;
import com.aiflow.dashboard.db.util.OrderUtil;
import com.aiflow.dashboard.core.util.IpUtil;
import com.aiflow.dashboard.wx.task.OrderUnpaidTask;
...
import static com.aiflow.dashboard.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    @Autowired
    private DashboardOrderGoodsService orderGoodsService;
    @Autowired
    private DashboardAddressService addressService;
    @Autowired
    private DashboardCartService cartService;
    @Autowired
    ...
    @Autowired
    private DashboardAftersaleService aftersaleService;
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<DashboardOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (DashboardOrder o : orderList) {
            ...
            DashboardGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<DashboardOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        DashboardOrder order = orderService.findById(userId, orderId);
        if (null == order) {
        ...
        List<DashboardOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            DashboardGrouponRules rules = grouponRulesService.findById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                ...
                // （2）不允许参加自己开团的团购
                DashboardGroupon groupon = grouponService.queryById(userId, grouponLinkId);
        ...
        // 收货地址
        DashboardAddress checkedAddress = addressService.query(userId, addressId);
        ...
        DashboardGrouponRules grouponRules = grouponRulesService.findById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 货品价格
        List<DashboardCart> checkedGoodsList = null;
        if (cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            DashboardCart cart = cartService.findById(cartId);
        ...
        for (DashboardCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            DashboardCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        // 添加订单商品表项
        for (DashboardCart cartGoods : checkedGoodsList) {
            // 订单商品
            DashboardOrderGoods orderGoods = new DashboardOrderGoods();
        ...
        for (DashboardCart checkGoods : checkedGoodsList) {
            ...
            DashboardGoodsProduct product = productService.findById(productId);
        ...
        // 如果使用了优惠券，设置优惠券使用状态
        if (couponId != 0 && couponId != -1) {
            DashboardCouponUser couponUser = couponUserService.findById(userCouponId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            DashboardGroupon groupon = new DashboardGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                DashboardGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        DashboardOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        DashboardGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            DashboardGrouponRules grouponRules = grouponRulesService.findById(groupon.getRulesId());
            ...
            List<DashboardGroupon> grouponList = grouponService.queryJoinRecord(groupon.getGrouponId());
            if (groupon.getGrouponId() != 0 && (grouponList.size() >= grouponRules.getDiscountMember() - 1)) {
                for (DashboardGroupon grouponActivity : grouponList) {
                    grouponActivity.setStatus(GrouponConstant.STATUS_SUCCEED);
                    grouponService.updateById(grouponActivity);
                }

                DashboardGroupon grouponSource = grouponService.queryById(groupon.getGrouponId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<DashboardOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        DashboardOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        DashboardOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        DashboardComment comment = new DashboardComment();
