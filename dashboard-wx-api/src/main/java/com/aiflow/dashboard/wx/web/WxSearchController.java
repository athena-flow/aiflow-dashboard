...
import com.aiflow.dashboard.db.service.DashboardKeywordService;
import com.aiflow.dashboard.db.service.DashboardSearchHistoryService;
...
public class WxSearchController {
    ...
    @Autowired
    private DashboardKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        DashboardKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<DashboardKeyword> hotKeywordList = keywordsService.queryHots();

        List<DashboardSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<DashboardKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (DashboardKeyword key : keywordsList) {
