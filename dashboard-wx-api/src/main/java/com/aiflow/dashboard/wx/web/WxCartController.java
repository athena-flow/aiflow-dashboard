...
import static com.aiflow.dashboard.wx.util.WxResponseCode.GOODS_NO_STOCK;
import static com.aiflow.dashboard.wx.util.WxResponseCode.GOODS_UNSHELVE;
...
public class WxCartController {
    ...
    public Object index(@LoginUser Integer userId) {
        ...
        List<DashboardCart> list = cartService.queryByUid(userId);
        List<DashboardCart> cartList = new ArrayList<>();
        // TODO
        // 如果系统检查商品已删除或已下架，则系统自动删除。
        // 更好的效果应该是告知用户商品失效，允许用户点击按钮来清除失效商品。
        for (DashboardCart cart : list) {
            DashboardGoods goods = goodsService.findById(cart.getGoodsId());
        ...
        for (DashboardCart cart : cartList) {
     ...
     */
    @PostMapping("add")
    public Object add(@LoginUser Integer userId, @RequestBody DashboardCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        //判断购物车中是否存在此规格商品
        DashboardCart existCart = cartService.queryExist(goodsId, productId, userId);
        if (existCart == null) {
            //取得规格的信息,判断规格库存
     ...
     */
    @PostMapping("fastadd")
    public Object fastadd(@LoginUser Integer userId, @RequestBody DashboardCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
     ...
     */
    @PostMapping("update")
    public Object update(@LoginUser Integer userId, @RequestBody DashboardCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        Integer productId = cart.getProductId();
        ...
        //判断是否存在该订单
        // 如果不存在，直接返回错误
        DashboardCart existCart = cartService.findById(userId, id);
        if (existCart == null) {
    ...
    public Object goodscount(@LoginUser Integer userId) {
        ...
        int goodsCount = 0;
        List<DashboardCart> cartList = cartService.queryByUid(userId);
        for (DashboardCart cart : cartList) {
            goodsCount += cart.getNumber();
        }
    ...
    public Object checkout(@LoginUser Integer userId, Integer cartId, Integer addressId, Integer couponId, Integer userCouponId, Integer grouponRulesId) {
        ...
        // 收货地址
        DashboardAddress checkedAddress = null;
        if (addressId == null || addressId.equals(0)) {
            checkedAddress = addressService.findDefault(userId);
            // 如果仍然没有地址，则是没有收货地址
            // 返回一个空的地址id=0，这样前端则会提醒添加地址
            if (checkedAddress == null) {
                checkedAddress = new DashboardAddress();
        ...
        if (cartId == null || cartId.equals(0)) {
            ...
            DashboardCart cart = cartService.findById(userId, cartId);
            if (cart == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        for (DashboardCart cart : checkedGoodsList) {
        ...
        List<DashboardCouponUser> couponUserList = couponUserService.queryAll(userId);
        for(DashboardCouponUser couponUser : couponUserList){
            tmpUserCouponId = couponUser.getId();
            DashboardCoupon coupon = couponVerifyService.checkCoupon(userId, couponUser.getCouponId(), tmpUserCouponId, checkedGoodsPrice);
            if(coupon == null){
                continue;
            }
