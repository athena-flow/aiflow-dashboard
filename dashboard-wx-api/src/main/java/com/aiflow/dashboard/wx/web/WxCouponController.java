...
import com.aiflow.dashboard.wx.vo.CouponVo;
import com.aiflow.dashboard.wx.util.WxResponseCode;
...
public class WxCouponController {
    ...
    public Object list(@RequestParam(defaultValue = "1") Integer page,
        ...
        List<DashboardCoupon> couponList = couponService.queryList(page, limit, sort, order);
    ...
    public Object mylist(@LoginUser Integer userId,
        ...
        List<DashboardCouponUser> couponUserList = couponUserService.queryList(userId, null, status, page, limit, sort, order);
        List<CouponVo> couponVoList = change(couponUserList);
        return ResponseUtil.okList(couponVoList, couponUserList);
    }

    private List<CouponVo> change(List<DashboardCouponUser> couponList) {
        List<CouponVo> couponVoList = new ArrayList<>(couponList.size());
        for(DashboardCouponUser couponUser : couponList){
            Integer couponId = couponUser.getCouponId();
            DashboardCoupon coupon = couponService.findById(couponId);
    ...
    public Object selectlist(@LoginUser Integer userId, Integer cartId, Integer grouponRulesId) {
        ...
        for (CouponVo cv : couponVoList) {
            DashboardCoupon coupon = couponVerifyService.checkCoupon(userId, cv.getCid(), cv.getId(), checkedGoodsPrice);
    ...
    public Object receive(@LoginUser Integer userId, @RequestBody String body) {
        ...
        // 用户领券记录
        DashboardCouponUser couponUser = new DashboardCouponUser();
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        ...
        DashboardCoupon coupon = couponService.findByCode(code);
        if(coupon == null){
