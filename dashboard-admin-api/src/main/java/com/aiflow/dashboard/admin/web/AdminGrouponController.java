...
import com.aiflow.dashboard.admin.task.GrouponRuleExpiredTask;
...
public class AdminGrouponController {
    ...
    public Object listRecord(String grouponRuleId,
        ...
        List<DashboardGroupon> grouponList = grouponService.querySelective(grouponRuleId, page, limit, sort, order);

        List<Map<String, Object>> groupons = new ArrayList<>();
        for (DashboardGroupon groupon : grouponList) {
            try {
                Map<String, Object> recordData = new HashMap<>();
                List<DashboardGroupon> subGrouponList = grouponService.queryJoinRecord(groupon.getId());
    ...
    public Object list(String goodsId,
                       ...
                       @Sort @RequestParam(defaultValue = "add_time") String sort,
                       @Order @RequestParam(defaultValue = "desc") String order) {
        List<DashboardGrouponRules> rulesList = rulesService.querySelective(goodsId, page, limit, sort, order);
        return ResponseUtil.okList(rulesList);
    }

    private Object validate(DashboardGrouponRules grouponRules) {
    ...
    @PostMapping("/update")
    public Object update(@RequestBody DashboardGrouponRules grouponRules) {
        Object error = validate(grouponRules);
        if (error != null) {
            return error;
        }

        DashboardGrouponRules rules = rulesService.findById(grouponRules.getId());
        if(rules == null){
    ...
    @PostMapping("/create")
    public Object create(@RequestBody DashboardGrouponRules grouponRules) {
        Object error = validate(grouponRules);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody DashboardGrouponRules grouponRules) {
