...
import static com.aiflow.dashboard.admin.util.AdminResponseCode.*;
...
public class AdminOrderService {
    ...
    public Object list(Integer userId, String orderSn, LocalDateTime start, LocalDateTime end, List<Short> orderStatusArray,
        ...
        List<DashboardOrder> orderList = orderService.querySelective(userId, orderSn, start, end, orderStatusArray, page, limit,
                sort, order);
        return ResponseUtil.okList(orderList);
    }

    public Object detail(Integer id) {
        DashboardOrder order = orderService.findById(id);
        List<DashboardOrderGoods> orderGoods = orderGoodsService.queryByOid(id);
     ...
     * 2. 管理员登录dashboard管理后台点击退款操作进行订单状态修改和商品库存回库
     *
    ...
    public Object reply(String body) {
        ...
        // 目前只支持回复一次
        DashboardComment comment = commentService.findById(commentId);
        if(comment == null){
            return ResponseUtil.badArgument();
        }
