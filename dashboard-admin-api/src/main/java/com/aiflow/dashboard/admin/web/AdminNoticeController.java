...
import static com.aiflow.dashboard.admin.util.AdminResponseCode.NOTICE_UPDATE_NOT_ALLOWED;
...
public class AdminNoticeController {
    ...
    public Object list(String title, String content,
        ...
        List<DashboardNotice> noticeList = noticeService.querySelective(title, content, page, limit, sort, order);
        return ResponseUtil.okList(noticeList);
    }

    private Object validate(DashboardNotice notice) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody DashboardNotice notice) {
        ...
        // 2. 添加管理员通知记录
        List<DashboardAdmin> adminList = adminService.all();
        ...
        for(DashboardAdmin admin : adminList){
    ...
    public Object read(@NotNull Integer id) {
        DashboardNotice notice = noticeService.findById(id);
        List<DashboardNoticeAdmin> noticeAdminList = noticeAdminService.queryByNoticeId(id);
    ...
    @PostMapping("/update")
    public Object update(@RequestBody DashboardNotice notice) {
        Object error = validate(notice);
        if (error != null) {
            return error;
        }
        DashboardNotice originalNotice = noticeService.findById(notice.getId());
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody DashboardNotice notice) {
        // 1. 删除通知管理员记录
